...
class ChessBoard
	...
	def get_moves_for_active_player
		game.active_player.pieces.flat_map do |active_player_piece|
			# puts active_player_piece
			active_player_piece.gather_legal_moves
			# puts rv.length
			# rv
		end
	end

	def initialize_with_fen_obj(game_object, fen_obj)
		...
		8.times do |file|
			board_array_cells = board_array_file_fen.split(//)
			rank = 0
	...
	def initialize( game_object, layout = nil, run_init = true )
		#needs to be associated with game
		if run_init
			self.game = game_object


			#needs to create a structure for the board
			self.board_array = Array.new(8) {Array.new(8)}

			#needs to create and place the pices
			layout ||= DefaultBoard

			layout.reverse.each_with_index do |rowStr, file|
				row = rowStr.split(//)
				row.each_with_index do |piece_char, rank|
					piece_player = /[[:upper:]]/.match(piece_char) ? self.game.black_game_player : self.game.white_game_player
					if piece_char == '-'
						self.board_array[file][rank] = nil
					else
						piece_class = CharToPieceClassnameMap[piece_char.upcase]
						# puts "problem #{piece_char.to_i} at #{rank}, #{file}" if piece_class == nil
						currentPosition = Position.new(rank,file)
						# puts piece_char+currentPosition.short
						self.board_array[file][rank] = piece_class.new(piece_player, self, currentPosition, game.piece_counter)
						game.piece_counter += 1
					end
				end
			end
		end
	...
	def clone(ng)
		nb = ChessBoard.new(nil, nil, false)
		nb.game = ng
		nb.board_array = Array.new(8) {Array.new(8)}
		self.board_array.each_with_index do |row, file|
			row.each_with_index do |piece, rank|
				if piece
					# puts "rank:#{rank} file:#{file}"
					# puts "-----------"
					nb.board_array[file][rank] = piece.class.new(
						piece.player.white ? ng.white_game_player : ng.black_game_player,
						nb,
						piece.position.clone,
						piece.piece_id_for_game )
				else
					nb.board_array[file][rank] = nil
				end
			end
		end
		# puts self
	...
	def endangers_king?(move)
		...
		retval = game.simulated_game.board.in_check?(game.active_player.opponent)
